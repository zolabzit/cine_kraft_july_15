...
function TvShows() {
  ...
  const filteredFictionSeries =
    selectedGenres.length === 0
      ? Fiction
      : Fiction.filter((series) => selectedGenres.includes(series.genre));

  const filteredNonFictionSeries =
    selectedGenres.length === 0
      ? NonFiction
      : NonFiction.filter((series) => selectedGenres.includes(series.genre));
  ...
  return (
    <div>
      ...
      <div className="about">
        <Container>
          <h1 style={{ paddingTop: "8%" }}>TV Shows</h1>
          <div>
            <Row>
      ...
      <div className="card">
        <TabView>
          <TabPanel header="Fiction">
            <div>
              <Container>
                <Row>
                  {filteredFictionSeries.map((series) => (
                    <Col
                      style={{ paddingTop: "2%" }}
                      key={series.id}
                      xs={12}
          ...
          <TabPanel header="Non-Fiction">
            <div>
              <Container>
                <Row>
                  {filteredNonFictionSeries.map((series) => (
                    <Col
                      style={{ paddingTop: "2%" }}
                      key={series.id}
                      xs={12}
     ...
     <Modal show={showFilterModal} onHide={handleFilterModalClose}>
        ...
        <Modal.Body>
          <Form>
            {Array.from(new Set(Fiction.concat(NonFiction).map((series) => series.genre))).map((genre, index) => (
              <Form.Check
                key={index}
                type="checkbox"
