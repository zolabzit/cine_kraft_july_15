...
const AddMovieForm = ({ onSubmit }) => {
  ...
  // audioRights: values.audioRights ? values.audioRights.map(item => item.name) : [],
        // satelliteRights: values.satelliteRights ? values.satelliteRights.map(item => item.name) : [],
        // ottRights: values.ottRights ? values.ottRights.map(item => item.name) : [],
        // broadcastLinks: values.broadcastLinks,
        // ottLinks: values.ottLinks,
        // songs: values.songs.map((song) => ({
        //   // songNumber: values[`songNum_${song.key}`],
        //   // songName: values[`songName_${song.key}`],
        //   // singerMale: values[`singerMale_${song.key}`],
        //   // singerFemale: values[`singerFemale_${song.key}`],
        //   // lyricists: values[`lyricist_${song.key}`],
        //   // musicDirectors: values[`music${song.key}`],
        //   // choreographers: values[`choreographer${song.key}`],
        //   // award: values[`SongAwards${song.key}`],
        //   // remarks: values[`remarks${song.key}`],
        //   // issue: values[`issue${song.key}`],
        //   // descriptions: values[`descriptions${song.key}`],
        // })),
      };
...
const handleAddMember = (type) => {
  ...
  const handleRemoveMember = (index, type) => {
   ...
   const handleFormSubmit = (values) => {
    ...
    // Add logic here to handle form submission (e.g., API calls, state updates)
  };
                ...
                {songs.map((song, index) => (
                  <div key={song.key} style={{ marginBottom: "20px" }}>
                    <Row align="middle">
                      ...
                      <Form.Item
                          label={
                            <span className="philosopher-font">Male</span>
                          }
