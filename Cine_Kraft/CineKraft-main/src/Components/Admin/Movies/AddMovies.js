...
const [remakeRows, setRemakeRows] = useState([
  ...
  const [dubbedMovieOptions, setDubbedMovieOptions] = useState(["Movie 1", "Movie 2", "Movie 3"]);
  ...
  //   try {
  //     // Construct formData object from formValues state
  //     const formData = {
  //       dubbedMovieName: formValues.dubbedMovieName,
  //       dubbedLanguages: formValues.dubbedLanguages,
  //       remakeMovieName: formValues.remakeMovieName,
  //       remakeLanguages: formValues.remakeLanguages,
  //       importantLinks: formValues.importantLinks,
  //       link: formValues.link,
  //       socialPlatforms: formValues.socialPlatforms,
  //       platformLink: formValues.platformLink,
  ...
  //       // Optionally reset formValues state after successful submission
  //       setFormValues({
  //         dubbedMovieName: '',
  //         dubbedLanguages: [],
  //         remakeMovieName: '',
  ...
  //       });
  //     } else {
  //       // Handle unsuccessful request (status outside 200-299 range)
  //       message.error('Failed to save movie. Please try again.');
  //     }
  //   } catch (error) {
  //     // Handle errors thrown during axios request
  ...
  const handleCheckboxRemakeChange = (e) => {
  ...
  const handleCheckboxImportantLinksChange = (e) => {
    const { checked } = e.target;
    setImportantLinks(checked);
  };



  const handleCheckboxSocialMediaChange = (e) => {
    const { checked } = e.target;
    setShowSocialMediaLinks(checked);
  };

  const handleSocialPlatformChange = (value, index) => {
    // Update the selected platform in the specified social media link index
    const newSocialPlatforms = [...formData.socialPlatforms];
    newSocialPlatforms[index].platform = value;
    setFormData({ ...formData, socialPlatforms: newSocialPlatforms });
  };

  const handleSocialPlatformLinkChange = (e, index) => {
    // Update the link in the specified social media link index
    const { value } = e.target;
    const newSocialPlatforms = [...formData.socialPlatforms];
    newSocialPlatforms[index].link = value;
    setFormData({ ...formData, socialPlatforms: newSocialPlatforms });
  };

  const handleDeleteSocialPlatform = (index) => {
    // Remove the social media link at the specified index
    const newSocialPlatforms = [...formData.socialPlatforms];
    newSocialPlatforms.splice(index, 1);
    setFormData({ ...formData, socialPlatforms: newSocialPlatforms });
  };
  ...
  //       audioRights: selectedAudioRights,
  //       broadcastChannels: selectedSatelliteChannels,
  //       ottRights: selectedOTTPlatforms,
  //       broadcastLinks: values.satelliteLinks,
  //       ottLinks: values.links,
  //       dubbedMovieName: values.dubbedMovieName,
  //       dubbedLanguages: values.dubbedLanguages,
  //       remakeMovieName: values.remakeMovieName,
  //       remakeLanguages: values.remakeLanguages,
  //       importantLinks: values.importantLinks,
  //       link: values.link,
  ...
  //     if (response.status >= 200 && response.status < 300) {
  //       message.success('Movie saved successfully!');
  //       // Reset form fields after successful submission (assuming you have reset functions)
