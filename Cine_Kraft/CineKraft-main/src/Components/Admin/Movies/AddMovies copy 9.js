...
const AddMovieForm = ({ onSubmit }) => {
  ...
  const handleReleaseStatusChange = (checkedValues) => {
    // checkedValues will be an array of selected checkbox values
    if (Array.isArray(checkedValues)) {
      setReleaseStatus(checkedValues); // For Checkbox.Group
    } else {
      setReleaseStatus(checkedValues.target.value); // For Radio.Group
    }
  };
