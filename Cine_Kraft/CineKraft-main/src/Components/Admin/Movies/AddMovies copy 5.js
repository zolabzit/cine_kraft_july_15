...
const AddMovieForm = ({ onSubmit }) => {
  ...
  const handleReleaseStatusChange = (e) => {
    if (Array.isArray(e)) {
      // Checkbox.Group sends an array of checked values
      setReleaseStatus(e);
    } else if (e && e.target && e.target.value !== undefined) {
      // Assuming it's a radio button or some other input with a value attribute
      setReleaseStatus(e.target.value);
    } else {
      // Handle other cases or errors
      console.error('Unsupported event:', e);
    }
  };
