...
const AddMovieForm = ({ onSubmit }) => {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      // Send POST request to save movie
      const movieResponse = await axios.post(
        "http://localhost:8080/api/movies",
        newMovie
      );
      console.log("Movie saved successfully:", movieResponse.data);
  
      // Construct rights data object based on checkboxes and inputs
      const rightsData = {
        audioRights: values.audioRights || [],
        satelliteRights: values.satelliteRights || [],
        ottRights: values.ottRights || [],
        ottLinks: values.links || {}, // Assuming links for OTT rights
        satelliteLinks: values.satelliteLinks || {}, // Assuming links for Satellite rights
      };
  
      // Send POST request to save rights information
      const rightsResponse = await axios.post(
        "http://localhost:8080/api/rights",
        rightsData
      );
      console.log("Rights saved successfully:", rightsResponse.data);
  
      // Handle success (e.g., show success message, redirect, etc.)
    } catch (error) {
      console.error("Error saving movie or rights:", error);
